...
def add_local_panoseti_grpc_src() -> None:
    """Prefer the sibling panoseti_grpc/src checkout when it is present."""
    here = Path(__file__).resolve().parent
    candidates = (
        here / "panoseti_grpc" / "src",
        here.parent / "Codebase" / "panoseti_grpc" / "src",
    )
    for path in candidates:
        if (path / "panoseti_grpc" / "telemetry" / "client.py").exists():
            path_str = str(path)
...
def main() -> int:
    ...
    if not args.no_publish:
        # Import lazily so --no-publish can still read sensors on machines that
        # do not have panoseti_grpc installed or on PYTHONPATH.
        add_local_panoseti_grpc_src()
